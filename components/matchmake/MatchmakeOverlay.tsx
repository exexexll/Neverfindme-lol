...
export function MatchmakeOverlay({ isOpen, onClose, directMatchTarget }: MatchmakeOverlayProps) {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    // Super sensitive swipe (20px - easier than TikTok)
    if (deltaX < 60 && Math.abs(deltaY) > 20) {
      // Check if waiting
      const currentUserId = users[currentIndex]?.userId;
      const isWaiting = currentUserId && inviteStatuses[currentUserId] === 'waiting';
      if (isWaiting) return;
            ...
            <div className="max-w-2xl w-full text-center space-y-6 sm:space-y-8 px-4 sm:px-0">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 max-w-xl sm:max-w-2xl mx-auto">
                ...
                <motion.button
                  ...
                  className={`group relative rounded-2xl p-6 sm:p-8 transition-all ${
                    chatMode === 'video'
                      ? 'bg-[#ffc46a] shadow-2xl shadow-[#ffc46a]/20'
              ...
              <div className="space-y-4 pt-2">
                <motion.button
                  ...
                  className="w-full sm:w-auto mx-auto block rounded-xl bg-[#ffc46a] px-8 sm:px-12 py-3 sm:py-4 text-base sm:text-lg font-medium text-[#0a0a0c] hover:opacity-90 transition-opacity shadow-xl"
                >
        ...
        <div className="relative flex-1 flex items-center justify-center px-4" role="list" aria-label="Available users">
            ...
            {loading && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <div className="inline-block h-12 w-12 animate-spin rounded-full border-4 border-[#ffc46a] border-t-transparent" />
              </div>
            )}
          </div>
      </div>
